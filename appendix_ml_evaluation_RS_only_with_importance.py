...
Purpose
-------
Reproducible evaluation of monthly 1M-ahead *raw* returns using classical ML regressors:
    • Linear Regression (LR)
    • Random Forest (RF)
    • Gradient Boosting (GB)
    • Simple Ensemble = mean(RF, GB)

Methodology (concise)
---------------------
• Data: Yahoo Finance via yfinance. Daily OHLCV → monthly last-of-month 'Adj Close'.
• Features: monthly technicals + macro proxies (all **lagged by 1M**).
• Target: next-month log return (raw).
• Panel: MultiIndex (Date, Ticker) with one-hot ticker dummies; y winsorized at 1%/99%.
• Validation: TimeSeriesSplit walk-forward with per-fold RandomizedSearchCV (RS+WF).
• Reporting: per-ticker medians & pooled metrics (R², MAE, Hit Rate, IC) + plots.
